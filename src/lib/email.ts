...
/**
 * Whether an inbound message is itself machine-generated, and so must not be
 * auto-replied to.
 *
 * The original guard only pattern-matched the sender address (noreply, bounce,
 * mailer-daemon). An out-of-office comes from the person's real address, so it
 * sailed straight through and we auto-replied to a vacation responder — which is
 * how a customer receipt turned into "Re: Automatic reply: Receipt for invoice…".
 *
 * Header checks follow RFC 3834 (Auto-Submitted) plus the de-facto headers the
 * major providers set. The subject patterns are the backstop for senders that
 * set no headers at all.
 */
export function isAutoResponse(input: {
  from?: string | null;
  subject?: string | null;
  headers?: { Name: string; Value: string }[] | null;
}): boolean {
  const header = (name: string): string | null => {
    const h = input.headers?.find((x) => x.Name.toLowerCase() === name.toLowerCase());
  ...
  // De-facto headers set by Exchange, Gmail vacation responder, and friends.
  for (const h of ["X-Autoreply", "X-Autorespond", "X-Auto-Response-Suppress", "X-Failed-Recipients"]) {
  ...
  // Senders that set no headers at all — match the subject instead. Allows a
  // leading Re:/Fwd: because the prefix survives a round trip.
  const subject = (input.subject ?? "").trim();
  if (
    /^((re|fw|fwd)\s*:\s*)*(automatic reply|auto(matic)?[- ]response|auto[- ]?reply|out of (the )?office|ooo\b|away from (my|the) (office|desk|email)|on vacation|annual leave|absence|undeliverable|delivery status notification|mail delivery (failed|subsystem)|r[ée]ponse automatique|abwesenheit)/i.test(
      subject,
    )
  ) {
    return true;
  ...
  return /noreply|no-reply|do-?not-?reply|mailer-daemon|postmaster|bounce/.test(from);
}
